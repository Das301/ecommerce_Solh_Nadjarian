...
def submit_review():
    ...
    customer = json.loads(requests.get("http://localhost:3000/get_customer/"+user).content)
    if "error" not in customer and customer[2] == password:
        review = json.loads(requests.get("http://localhost:3000/get_review/"+user+"/"+info["good"]).content)
...
def update_review():
    ...
    if "error" not in customer and customer[2] == password:
        ...
        if review is not None:
            ...
            response = requests.post("http://localhost:3000/update", json=data)
            return response.content
        else:
...
def delete_review():
    ...
    if "error" not in customer and customer[2] == password:
        ...
        if review is not None:
            ...
            response = requests.post("http://localhost:3000/delete", json=data)
            return response.content
        else:
            return jsonify("No reviews exists for this product")
    else:
...
def admin_delete_review():
    ...
    admin = json.loads(requests.get("http://localhost:3000/get_admin/"+admin_user).content)
...
def get_review(user, good):
    ...
    :rtype: flask.Response
    """
    review = requests.get("http://localhost:3000/get_review/"+user+"/"+good).content
    if review is None:
...
def get_product_review(good):
    ...
    :rtype: flask.Response
    """
    reviews = requests.get("http://localhost:3000/get_reviews_product/"+good).content
...
def get_user_review(user):
    ...
    :rtype: flask.Response
    """
    reviews = requests.get("http://localhost:3000/get_reviews_user/"+user).content
    if len(json.loads(reviews))==0:
...
def flag_review():
    ...
    """
    info = request.get_json()
    response = requests.post("http://localhost:3000/flag", json=info)
    return response.content
