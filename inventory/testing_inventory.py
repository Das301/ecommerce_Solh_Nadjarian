...
def add_good():
    ...
    response = requests.post("http://localhost:3000/add_good", json=info)
...
def deduct_good():
    ...
    """
    info = request.get_json()
    response = requests.patch(f"http://localhost:3000/deduct_good/{info['id']}", json={"quantity": info["quantity"]})
    return response.content
...
def update_good():
    ...
    """
    info = request.get_json()
    response = requests.patch(f"http://localhost:3000/update_good/{info['id']}", json=info["updates"])
    return response.content
