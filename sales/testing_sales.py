...
def get_available_goods():
    ...
    """
    response = requests.get("http://localhost:3000/get_goods")
...
def get_good_details(id):
    ...
    :rtype: flask.Response
    """
    response = requests.get("http://localhost:3000/get_good/"+str(id))
    return response.content
...
def record_sales():
    ...
    price = json.loads(requests.get("http://localhost:3000/get_good_price/"+good).content)
    ...
    data = {"user": user,
            ...
            "quantity": quantity,
            "total": total_amount}
    response = requests.post("http://localhost:3000/perform_transaction", json=data)
